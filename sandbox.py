...
def add_reserve(content, author, time, hash):
    add_content = Bin(author=author, content=content, hash=hash, time=time)
    db.session.add(add_content)
    db.session.commit()
    db.session.refresh(add_content)
    content_info = {"author": author, "content": content, "time": time, "hash": hash}
    return short_url_info

def creation_engine(author, content):
    time = time_cal()
    hash = hash_gen_engine()
    content_info = add_reserve(content, author, time, hash)
    return content_info
...
def custom_creation_engine(author, content, hash):
    time = time_cal()
...
print(creation_engine("NoobScience", "YouTube"))